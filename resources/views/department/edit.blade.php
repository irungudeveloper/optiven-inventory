					...
					<form method="post">
						...
						<div class="form-group row">
						   ...
						   <input type="hidden" name="department_id" id="department_id" value=" {{ $department->id }} ">
						      <input type="text" class="form-control" id="department" placeholder="Name" name="department" value=" {{ $department->name }} ">
						   </div>
						  </div>
						<div class="form-group text-center">
							<input type="button" name="submit" class="btn btn-success pl-4 pr-4" value="UPDATE DEPARTMENT" id="create">
						</div>
					</form>
				</div>
			</div>
		...
		$('#create').on('click', function(e) 
		{
			   ...
			   $.ajax({
			   		...
			   		success:function(response)
			   		{
			   			...
			   			{
							swal.fire("Done!", "Department Updated" , "success");
						} 
			   		}
			   });
